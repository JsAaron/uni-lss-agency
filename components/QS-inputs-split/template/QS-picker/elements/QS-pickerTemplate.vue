	...
	<view :style="{ 'z-index': zIndex }" @touchmove.stop="voidFc()" @tap.prevent.stop="voidFc()">
		...
		>
			<view class="flex_column" :class="(mode || 'middle') + '_view'" :style="{ height: heightSize }">
				<view
					v-if="mode === 'bottom'"
					class="flex_row_between_c width100 padding20rpx"
					:style="{ 'font-size': titleSize, 'background-color': bgColor_title || '#f8f8f8' }"
				>
					<view
						class="flex_row_none_c width250rpx "
						:style="{ color: buttonSet.cancleColor || '#ADADAD' }"
						@tap="hide()"
					>
						{{ buttonSet.cancleName || '取消' }}
					</view>
					<view class="flex_row_c_c width250rpx" :style="{ color: titleColor || '#999' }">{{ title || '' }}</view>
					<view
						class="flex_row_e_c width250rpx"
				...
				<view
					...
					class="flex_row_c_c width100 padding20rpx"
					:style="{
						'font-size': titleSize,
